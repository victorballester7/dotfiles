...
function get_album_art {
    if [[ $(playerctl -l) == *"spotify"* ]]; then
        playerctl_metadata="spotify"
        url=$(playerctl -p $playerctl_metadata -f "{{mpris:artUrl}}" metadata)
        if [[ $url == "file://"* ]]; then
            album_art="${url/file:\/\//}"
...
function show_volume_notif {
    volume=$(get_mute)
    get_volume_icon
    if [[ $show_music_in_volume_indicator == "true" ]] && [[ $(playerctl -l) == *"spotify"* ]]; then
        playerctl_metadata="spotify"
        current_song=$(playerctl -p $playerctl_metadata -f "{{title}} - {{artist}}" metadata)
...
function show_music_notif {
    # check if 'playerctl -l' contains 'spotify' 
    if [[ $(playerctl -l) == *"spotify"* ]]; then
        playerctl_metadata="spotify"
        song_title=$(playerctl -p $playerctl_metadata -f "{{title}}" metadata)
        song_artist=$(playerctl -p $playerctl_metadata -f "{{artist}}" metadata)
        song_album=$(playerctl -p $playerctl_metadata -f "{{album}}" metadata)
...
# if this sleep gets killed, then return 1
sleep 2
if [[ $? -ne 0 ]]; then
    exit 1
fi
